...
一個專門給擺攤時遇到奧客、情緒快爆炸時用的即時輔助 PWA。

核心目標：**在情緒快失控的當下，幫助使用者快速穩住情緒，並把腦中的氣話轉換成禮貌但有界線的回應。**

使用場景：夜市擺攤、手可能髒、環境吵雜、需要單手快速操作。
...
- 6 種意圖情境：
  - `price_reject`：殺價 / 嫌貴
  - `quality_defend`：嫌品質差 / 指控黑心
  - `end_conversation`：要趕人 / 想叫對方滾
  - `wait_urge`：被催太慢
  - `rude_general`：對方罵人 / 人身攻擊
  - `refuse_request`：要拒絕不合理要求
- 每種情境 3 句輪替輸出（隨機）
